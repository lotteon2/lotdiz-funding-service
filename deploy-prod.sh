...
if ! kubectl get deployment funding-deployment &> /dev/null; then
  echo "Create new kubernetes deployment resources..."
  kubectl create -f ./deployment-prod.yml
else
  echo "Apply new docker image "
  kubectl set image deployments/funding-deployment funding-service=$ECR_REGISTRY/$AWS_ECR_REPOSITORY:$IMAGE_TAG --record
fi
...
if ! kubectl get deployment funding-deployment &> /dev/null; then
  ...
  if ! kubectl rollout status deployment/funding-deployment --watch=false | grep -q "successfully rolled out"; then
    echo "Rolling update has failed. Initiating rollback..."

    kubectl rollout undo deployment/funding-deployment
    kubectl rollout status deployment/funding-deployment --watch=true
    ...
    if ! kubectl get service funding-service &> /dev/null; then
